...
let DeleteCommentService = class DeleteCommentService {
    ...
    async execute({ user_id, comment_id }) {
        const user = await this.usersRepository.findById(user_id);
        if (!user) {
            throw new apollo_server_1.ApolloError('User cannot be found', '400');
        }
        if (!mongodb_1.ObjectId.isValid(comment_id)) {
            throw new apollo_server_1.ApolloError('Comment ID invalid', '400');
        }
        const comment = await this.commentsRepository.findByID(comment_id);
        if (!comment) {
            throw new apollo_server_1.ApolloError('Comment cannot be found', '400');
        }
...
DeleteCommentService = __decorate([
    ...
    __param(0, tsyringe_1.inject('CommentsRepository')),
