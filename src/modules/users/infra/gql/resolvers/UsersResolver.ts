...
export async function getUser(_: any, { id }: IGetUser): Promise<User> {
  const showProfileService = container.resolve(ShowProfileService);
  const user = await showProfileService.execute({ id });
  return classToClass(user);
}

export async function createUser(_: any, { data }: ICreateUser): Promise<User> {
...
export async function userSummary(_: any, { id }: IGetUser): Promise<ISummary> {
